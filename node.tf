...
sed -i s,CLUSTER_NAME,${var.name}-${var.stage},g /var/lib/kubelet/kubeconfig
...
resource "aws_launch_configuration" "node" {
  ...
  name_prefix                 = "terraform-eks-${var.name}-${var.stage}"
...
resource "aws_autoscaling_group" "node" {
  ...
  tag {
    key                 = "Name"
    value               = "terraform-eks-${var.name}-${var.stage}"
    propagate_at_launch = true
  }

  tag {
    key                 = "kubernetes.io/cluster/${var.name}-${var.stage}"
    value               = "owned"
    propagate_at_launch = true
  }
}
