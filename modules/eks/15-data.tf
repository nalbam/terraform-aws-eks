...
data "template_file" "cluster_role_binding_admin_subjects" {
  count = length(var.map_users)
  template = file(
    "${path.module}/template/cluster_role_binding_admin_subjects.yaml.tpl",
  )
...
data "template_file" "cluster_role_binding_admin" {
  template = file(
    "${path.module}/template/cluster_role_binding_admin.yaml.tpl",
  )

  vars = {
    subjects = join(
      "",
      data.template_file.cluster_role_binding_admin_subjects.*.rendered,
    )
  }
}
...
data "template_file" "cluster_role_binding_view_subjects" {
  count = length(var.map_users)
  template = file(
    "${path.module}/template/cluster_role_binding_view_subjects.yaml.tpl",
  )
...
data "template_file" "cluster_role_binding_view" {
  ...
  vars = {
    subjects = join(
      "",
      data.template_file.cluster_role_binding_view_subjects.*.rendered,
    )
  }
}
